...
SELECT ST_SRID(geom) FROM public.nationalparks;

CREATE TABLE public.uk_lake_district AS
SELECT ST_Clip(a.rast, b.geom, true)
FROM  public.uk_250k AS a, public.nationalparks AS b
...
-- Zapisywanie
SELECT lo_export(loid, 'E:/STUDIA/semestr5/BazyDanychPrzestrzennych/cw8/uk_lake_district.tif')
FROM tmp_out;
...
SELECT lo_export(loid, 'D:/bdp/uk_lake_district_ndwi.tif')
FROM tmp_out;
